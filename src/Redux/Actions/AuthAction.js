...
export const GetSetsFunction = () => {
  ...
  return async (dispatch, state) => {
    try {
      // Cards
      let response = await axios.get(
        `${state().AuthReducer.baseUrl}set?subject=cards&limit=5000`,
      );
      ...
      response = await axios.get(
        `${state().AuthReducer.baseUrl}set?subject=vocabulary&limit=5000`,
      );
      dispatch(Vocabulary(response.data, 'premium'));
...
export const GetAllQuizzes = () => {
  ...
  return async (dispatch, state) => {
    try {
      let response = await axios.get(`${state().AuthReducer.baseUrl}quiz?limit=5000`);
      dispatch(Quiz(response.data.results));
    } catch (err) {
      console.log(`Errrr------->`, err.response.data.message);
      FlashMessage({
...
export const GetCardSets = subject => {
  ...
  return async (dispatch, state) => {
    try {
      // Cards
      let response = await axios.get(
        `${state().AuthReducer.baseUrl}set?subject=${subject}&limit=5000`,
      );
...
export const EraseAllData = (userId,navigation)=>{
  return async (dispatch, state) =>{
    try{
